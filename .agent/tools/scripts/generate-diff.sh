...
# Detect conversation ID from artifacts directory
BRAIN_DIR="${ANTIGRAVITY_BRAIN_DIR:-.agent/brain}"
...
if [ "$CURRENT_BRANCH" = "$DEFAULT_BRANCH" ]; then
  ...
  git diff HEAD -- . ':!.agent' > "$OUTPUT_FILE"
else
  echo -e "${GREEN}✓ Comparing $CURRENT_BRANCH with $DEFAULT_BRANCH${NC}"
  # Get diff between current branch and default branch, excluding .agent/
  git diff "$DEFAULT_BRANCH"...HEAD -- . ':!.agent' > "$OUTPUT_FILE"
fi

# Check if diff is empty
if [ ! -s "$OUTPUT_FILE" ]; then
  echo -e "${YELLOW}⚠️  No changes detected (excluding .agent/)${NC}"
...
git diff --name-only "$DEFAULT_BRANCH"...HEAD -- . ':!.agent' 2>/dev/null || git diff --name-only HEAD -- . ':!.agent'
echo ""
